...
UPLOADS_PATH = Path.cwd() / "Semester_2" / "flask-file-upload-2" / "static" / "uploads"
...
@app.route('/')
def index():
    with open('Semester_2/flask-file-upload-2/static/temp.csv', mode='r') as file:
...
@app.route('/api/data')
def get_data():
    with open('static/temp.csv', mode='r') as file:
        csv_reader = csv.reader(file)
        header = next(csv_reader)
        print(header)
        list1 = []
...
def add_data():
    ...
    with open('static/temp.csv', 'a+', encoding="utf-8") as file:
